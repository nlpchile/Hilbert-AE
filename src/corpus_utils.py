...
class Corpus(object):
    ...
    def save_h5py(self, path_to_ofile, path_to_ifile=None):
        ...
        dset1 = f.create_dataset(
            "sequence", (current_buffer_size, self.max_sequence_length),
            maxshape=(None, self.max_sequence_length),
            dtype="int32")
        dset2 = f.create_dataset(
            "mask", (current_buffer_size, self.max_sequence_length),
            maxshape=(None, self.max_sequence_length),
            dtype="uint8")
        ...
        if path_to_ifile == None:
            if len(self.corpus) == 0:
                ...
                for text in self.corpus:
                    dset1, dset2, current_buffer_allocation, current_buffer_size = helper_h5py(
                        dset1, dset2, self.encode(text),
                        current_buffer_allocation, current_buffer_size,
    ...
    def save_corpus_parameters(self, outfile):
        json_dict = {
            "max_sequence_dataset": self.max_sequence_dataset,
...
class H5PytorchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key, dtype in self.keys_types.items():
            items.append(
                torch.Tensor(self.h5pyfile[key][index, :]).type(dtype=dtype))
        return items
